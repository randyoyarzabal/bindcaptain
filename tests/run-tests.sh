...
test_container_startup() {
    ...
    local test_named_conf="$TEST_CONFIG_DIR/test.conf"
    mkdir -p "$TEST_CONFIG_DIR"
...
EOF
    ...
    if ! timeout 30s $container_cmd run --rm --name "$CONTAINER_NAME-dryrun" \
        --entrypoint bash \
        -v "$test_named_conf:/test.conf:ro" \
        "$IMAGE_NAME" -c "named-checkconf /test.conf" >/dev/null 2>&1; then
...
# Requires BC_LIVE_TESTS=1. Set BC_HOST when running from a workstation (e.g. root@dns.example.com).
test_bc_live_crud() {
...
show_help() {
    ...
    echo "  BC_HOST              SSH target for live tests (e.g. root@dns.example.com)"
    echo
    echo "Example:"
    echo "  ./run-tests.sh"
    echo "  ./tests/driver.sh                    # fast default (skips container build)"
    echo "  ./tests/driver.sh --live --host root@dns.example.com"
    echo "  SKIP_CONTAINER_TESTS=1 ./run-tests.sh"
    echo "  BC_LIVE_TESTS=1 BC_HOST=root@dns.example.com ./run-tests.sh"
}
