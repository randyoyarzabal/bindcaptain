...
print_bindcaptain_header() {
    print_header "⚓BindCaptain ${BINDCAPTAIN_VERSION}" "BIND DNS Container"
}
...
prepare_config() {
    ...
    # Resolve any symlinks first: `chown -R` does not traverse a symlinked
    # top-level path argument by default and silently no-ops, leaving zone
    # files unreadable by named. Always operate on the real path.
    local cfg_real
    cfg_real="$(readlink -f "$BINDCAPTAIN_CONFIG_PATH" 2>/dev/null || echo "$BINDCAPTAIN_CONFIG_PATH")"

    # Ensure proper permissions on source config files
    sudo chown -R named:named "$cfg_real"
    sudo chmod 644 "$cfg_real/named.conf"
    sudo chmod 644 "$cfg_real"/*/*.db 2>/dev/null || true

    # Create necessary directories for container operation
    mkdir -p "$cfg_real/data" "$cfg_real/logs"
    sudo chown -R named:named "$cfg_real/data" "$cfg_real/logs"
...
# Auto-detect bind IP from named.conf. Returns the first IPv4 address listed in
# `listen-on { ... }`. Returns "any" if nothing concrete is found; the caller
# decides what that means in its networking mode.
detect_bind_ip() {
...
# Network mode for the container. Two values:
#   bridge (default) — podman/CNI bridge network. Container has its own
#                      private IP; ports are forwarded from the host's listen
#                      address. Best isolation. CAVEAT: NOTIFY/AXFR to slaves
#                      use the container's bridge IP as the bind() source.
#                      If the SOA/NS record points at the host IP and you
#                      configure notify-source/transfer-source = host IP,
#                      named will fail with EADDRNOTAVAIL and silently drop
#                      the NOTIFY. See docs/networking.md.
#   host             — share the host's network namespace. Container binds
#                      directly to the host's interfaces. NOTIFY/AXFR have
#                      a real source IP that matches the SOA/NS. Recommended
#                      for any deployment that must serve as a primary to
#                      external secondaries.
#
# Override per-deployment with: export BINDCAPTAIN_NETWORK_MODE=host
BINDCAPTAIN_NETWORK_MODE="${BINDCAPTAIN_NETWORK_MODE:-bridge}"
...
run_container() {
    ...
    case "$BINDCAPTAIN_NETWORK_MODE" in
        host)
            network_args=(--network host)
            print_status "info" "Network mode: host (container shares host netns; binds directly)"
            ;;
        bridge|"")
            if [ "$bind_ip" = "any" ]; then
                # No specific listen-on; bind to all interfaces on the host.
                network_args=(-p "53:53/tcp" -p "53:53/udp")
            else
                network_args=(-p "${bind_ip}:53:53/tcp" -p "${bind_ip}:53:53/udp")
            fi
            print_status "info" "Network mode: bridge (forwarding host:${bind_ip}:53 -> container)"
            print_status "info" "  Tip: if this server NOTIFIES external secondaries, use host mode (BINDCAPTAIN_NETWORK_MODE=host)"
            ;;
        *)
            print_status "error" "BINDCAPTAIN_NETWORK_MODE='$BINDCAPTAIN_NETWORK_MODE' is invalid (use: host | bridge)"
            exit 1
            ;;
    esac

    print_status "info" "Starting BIND DNS container..."
...
show_info() {
    ...
    echo "  Network mode: ${BINDCAPTAIN_NETWORK_MODE}"
...
show_help() {
    ...
    echo "  BINDCAPTAIN_NETWORK_MODE     - Container network mode: bridge (default) | host"
    echo "                                 Use 'host' if this server NOTIFIES external secondaries"
    echo "                                 (see docs/networking.md)"
